...
class FetchController extends AbstractController
{
    ...
    public function __construct(PatientRepository $patientRepository,ContentRepository $contentRepository, EntityManagerInterface $entityManager,Security $security){
    ...
    /**
     * @Route("/fetch_cont", name="fetch_content", methods={"POST"})
     * @param Request $request
     ...
     * @throws NonUniqueResultException
     */
    public function fetchContent(Request $request, ContentRepository $repository):JsonResponse
    {
        if ($request->getMethod() == 'POST')
        {
            $draw =  intval($request->request->get('draw'));
        ...
        $user = $this->security->getUser();
        $results = $this->contentRepository->findDataTable($start, $length,$search,$orders,$columns,1);
        $total_objects_count = $this->contentRepository->countElements(1);
